...
export const Connect = () => {
  ...
  return (
    <div
      ref={ref}
      className={`w-full max-w-1/2 max-md:max-w-full bg-fourth rounded-2xl h-[inherit] ${
    ...
    >
      <div className="py-10 px-8 flex flex-col gap-y-10 body-small-r text-body-small-d max-md:text-body-small-m">
        <p className="subtitle1-b text-subtitle-1-d max-md:text-subtitle-1-m  bg-gradient-to-l from-violet-600 to-fuchsia-600 text-transparent bg-clip-text">
          {t("connectTitle")}
        </p>
          ...
          {connectSvgs.map((svg, id) => (
            <Link href={svg.href} key={id} target="_blank">
              <div className="flex flex-col items-center gap-y-3">
                ...
                <p className="small-r text-small-d max-md:text-small-m">
                  {svg.name}
                </p>
              </div>
            </Link>
